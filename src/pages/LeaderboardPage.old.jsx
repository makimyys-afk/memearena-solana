...
import leaderboardData from '../assets/leaderboard-sample.json'
          ...
          {leaderboardData.slice(0, 3).map((player, index) => {
            ...
            return (
              <motion.div
                ...
                className={`relative ${actualIndex === 0 ? 'md:-mt-8' : 'md:mt-8'}`}
              >
                <div className={`bg-gradient-to-br ${getMedalColor(actualPlayer.rank)} p-1 rounded-2xl`}>
                  <div className="bg-black/90 rounded-2xl p-6 text-center">
                    {/* Medal */}
                    <div className="relative mb-4">
                      <img 
                        src={getMedalIcon(actualPlayer.rank)}
                        alt={`Rank ${actualPlayer.rank}`}
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-green-500 font-bold">{actualPlayer.wins}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400">Losses</span>
                        <span className="text-red-500 font-bold">{actualPlayer.losses}</span>
                      </div>
                      <div className="flex justify-between items-center pt-2 border-t border-gray-700">
                        <span className="text-gray-400">Earned</span>
                        <span className="text-yellow-500 font-bold flex items-center gap-1">
                          <img src="/src/assets/ui-assets/sol_coin_icon.png" alt="SOL" className="w-4 h-4" />
                          {actualPlayer.earnedSol} SOL
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className="text-yellow-500 font-bold">{player.earnedSol}</span>
                      </div>
                    </td>
                  </motion.tr>
                ))}
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <h3 className="text-3xl font-bold text-white mb-2">
              {leaderboardData.reduce((sum, p) => sum + p.wins + p.losses, 0)}
            </h3>
          ...
          >
            ...
            <h3 className="text-3xl font-bold text-white mb-2">
              {leaderboardData.reduce((sum, p) => sum + p.earnedSol, 0).toFixed(1)} SOL
            </h3>
